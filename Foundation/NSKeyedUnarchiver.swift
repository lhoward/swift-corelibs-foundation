...
public class NSKeyedUnarchiver : NSCoder {
    ...
    private static func _unescapeKey(key : String) -> String {
        if key.hasPrefix("$") {
     ...
     */
    internal func _decodeArrayOfObjectsForKey(key: String, withBlock block: (Any) -> Void) throws {
    ...
    public override func containsValueForKey(key: String) -> Bool {
        return _decodeValue(forKey: key) != nil
    }
    ...
    private func _decodeValueOfObjCType(type: NSObjCType, at addr: UnsafeMutablePointer<Void>) {
        ...
        case .CharPtr:
            if let ns = decodeObject() as? NSString {
                ns.getCString(UnsafeMutablePointer<CChar>(addr), maxLength: ns.length, encoding: NSUTF8StringEncoding)
            }
            break
        default:
